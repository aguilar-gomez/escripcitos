...
foreach my $id (keys %seq) {
	...
	if (length($seq{$id}) > $longest){
		$longest= length($seq{$id});
		$longest_seqid = $id;
	}
	if (length($seq{$id}) < $shortest){
		$shortest = length($seq{$id});
		$shortest_seqid = $id;
	}
	if (length($seq{$id}) > 400){
		#print "$id\n";
	}
}
#print "\n $longest\n";
...
print "Longest transcript:\t$longest bp\n";
print "Longest Sequences:\t$longest_seqid\n";
print "Shortest transcripts:\t$shortest bp\n";
print "Shortest Sequence:\t$shortest_seqid\n";
